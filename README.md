...
🚀 Kurulum
En güncel sürümü Sürümler (Releases) sayfasından indirin.
Namify-Renamer-Setup-x.x.x.exe dosyasını çalıştırın.
Kurulum tamamlandığında uygulama otomatik olarak açılacaktır.
📖 Kullanım Kılavuzu
Dosya Ekle: Dosyaları sürükleyip uygulama alanına bırakın veya klasör seçin.
Kural Seç: Sol panelden (veya üstten) aktif etmek istediğin kuralları seç.
Değer Gir: İlgili alanları (Bul, Değiştir, Liste vb.) doldur.
Önizle: Sağ/Alt paneldeki canlı önizlemeyi kontrol et.
Uygula: "Yeniden Adlandır" butonuna bas ve işlemin tadını çıkar!
...
Geliştiren: Antigravity (ve işbirliğiyle!) GitHub: knutolof06/namify-renamer
